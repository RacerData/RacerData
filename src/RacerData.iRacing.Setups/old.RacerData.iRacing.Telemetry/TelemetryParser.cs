...
namespace RacerData.iRacing.Telemetry
{
    public class TelemetryParser : ITelemetryParser
    {
        ...
        #region public   
        public async Task<ITelemetryData> ParseTelemetrySessionAsync(byte[] telemetryBytes)
        {
            return await ParseTelemetrySessionAsync(telemetryBytes, IbtParseOptions.All);
        }
        public async Task<ITelemetryData> ParseTelemetrySessionAsync(byte[] telemetryBytes, IbtParseOptions options)
        {
        ...
        #region protected
        protected virtual async Task<ITelemetryData> ParseTelemetrySession(byte[] telemetryBytes, IbtParseOptions options)
        {
            var s = await Task.Run(() =>
            {
                var session = new TelemetryData();
        ...
        protected virtual IList<IFrame> ParseFrames(byte[] telemetryBytes, IList<IFieldDefinition> fieldDefinitions, ref int dataStartIdx)
        {
            dataStartIdx += 1;
            ...
            return ParseValues(frameBytes, fieldDefinitions);
        }
        ...
        protected virtual IList<IFrame> ParseValues(byte[] framesSectionBytes, IList<IFieldDefinition> fieldDefinitions)
        {
            ...
            while (true)
            {
                // iterates through each frame of data inthe byte array
        ...
        #region laps
        protected virtual IList<ILapInfo> ParseLaps(TelemetryData session)
        {
        ...
        protected virtual double GetTrackLengthInMiles(TelemetryData session)
        {
